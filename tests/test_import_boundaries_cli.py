...
def test_production_release_verifies_prebuilt_assets_before_publish() -> None:
    ...
    assert "anygeometry-{version}-py3-none-any.whl" in workflow
    assert "path.is_symlink()" in workflow


def _release_verifier() -> str:
    workflow = (ROOT / ".github/workflows/publish-release-assets.yml").read_text(encoding="utf-8")
    return workflow.split("          python - <<'PY'\n", 1)[1].split("\n          PY", 1)[0].replace("          ", "", 1).replace("\n          ", "\n")


def _run_release_verifier(tmp_path: Path, *, tag: str = "v0.4.1", wheel: str = "anygeometry-0.4.1-py3-none-any.whl", digest_ok: bool = True) -> subprocess.CompletedProcess[str]:
    release = tmp_path / "release"
    release.mkdir(parents=True)
    files = {wheel: b"wheel", "anygeometry-0.4.1.tar.gz": b"sdist"}
    ...
    return subprocess.run([sys.executable, "-c", _release_verifier()], cwd=tmp_path, env=environment, capture_output=True, text=True, check=False)


def test_release_verifier_rejects_wrong_tag_name_and_hash(tmp_path: Path) -> None:
    assert _run_release_verifier(tmp_path / "valid").returncode == 0
    assert _run_release_verifier(tmp_path / "tag", tag="v0.4.0").returncode != 0
    assert _run_release_verifier(tmp_path / "name", wheel="other-0.4.1-py3-none-any.whl").returncode != 0
    assert not _run_release_verifier(tmp_path / "hash", digest_ok=False).returncode == 0
