...
@dataclass(frozen=True)
class Spline:
    """Bezier spline through topology-owned control vertices.

    This deliberately small representation keeps spline support lightweight
    and deterministic.  More elaborate interpolation policies can be added
    without changing edge identity or the topology model.
    """
...
def sample_spline(control_points: np.ndarray, t: np.ndarray) -> np.ndarray:
    """Sample a Bezier spline using stable de Casteljau interpolation."""

    points = np.asarray(control_points, dtype=float)
    parameters = np.asarray(t, dtype=float).reshape(-1)
...
def spline_tangent(control_points: np.ndarray, t: float) -> np.ndarray:
    """Unit tangent of a Bezier spline."""
    ...
    tangent = (
        derivative[0]
        if len(derivative) == 1
        else sample_spline(derivative, np.array([float(t)]))[0]
    )
    length = float(np.linalg.norm(tangent))
    if length <= 0.0:
        raise ValueError("cannot take the tangent of a degenerate spline")
